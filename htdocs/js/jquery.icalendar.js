...
function iCalendar() {
	this._defaults = {
		...
		icons: 'img/icalendar.png', // Horizontal amalgamation of all site icons
...
$.extend(iCalendar.prototype, {
	...
	_updateICalendar: function(target, settings) {
		...
		var addSite = function(site, calId) {
			...
			if (site.url == 'echo') {
				anchor.click(function() {
					...
					//_gaq.push(['_trackEvent', 'Calendar Reminder', 'Click', settings.location]);
					setTimeout(function() {
		...
		var list = $('<span class="icalendar_list' +
			(settings.compact ? ' icalendar_compact' : '') + '"></span>');
